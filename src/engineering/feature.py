...
def create_interaction_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data["total_user_interactions"] = data.groupby("user_id")["item_id"].transform(
        "count"
    )
    data["user_item_interactions"] = data.groupby(["user_id", "item_id"])[
        "item_id"
    ].transform("count")

    # Item popularity
    data["normalized_popularity"] = data.groupby("item_id")["user_id"].transform(
        "count"
    )

    # User diversity metrics
    data["user_unique_items"] = data.groupby("user_id")["item_id"].transform("nunique")
    data["user_unique_item_types"] = data.groupby("user_id")["item_type"].transform(
        "nunique"
    )
    data["user_unique_segments"] = data.groupby("user_id")["segment"].transform(
        "nunique"
    )
    data["user_unique_behavior_segments"] = data.groupby("user_id")[
        "beh_segment"
    ].transform("nunique")
...
def create_temporal_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data["weekend_interaction_rate"] = data.groupby("user_id")["is_weekend"].transform(
        "mean"
    )
    data["month_start_interaction_rate"] = data.groupby("user_id")[
        "is_month_start"
    ].transform("mean")
    data["month_end_interaction_rate"] = data.groupby("user_id")[
        "is_month_end"
    ].transform("mean")
...
def create_sequence_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data["prev_item_id"] = (
        data.groupby("user_id")["item_id"].shift(1).fillna("no_prev_item")
    )
    data["prev_item_type"] = (
        data.groupby("user_id")["item_type"].shift(1).fillna("no_action")
    )

    # Time between interactions
    data["days_since_last_interaction"] = (
        data.groupby(["user_id", "item_id"])["int_date"].diff().dt.days.fillna(-1)
    )
...
def parse_arguments() -> Namespace:
    ...
    parser = ArgumentParser(
        description="Feature Engineering Pipeline for Recommendation System"
    )
    parser.add_argument(
        "--process", type=str, required=True, help="Path to processed data input file"
    )
    parser.add_argument(
        "--train", type=str, required=True, help="Path to training data output file"
    )
    parser.add_argument(
        "--test", type=str, required=True, help="Path to test data output file"
    )
